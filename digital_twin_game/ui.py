...
def draw_text(
    ...
    image = get_font(size, bold).render(text, True, color or COLORS["text"])
    rect = image.get_rect()
...
def draw_bar(
    ...
    pygame.draw.rect(surface, (26, 34, 52), rect, border_radius=rect.height // 2)
    ...
    pygame.draw.rect(surface, (72, 91, 118), rect, 1, border_radius=rect.height // 2)
    if label:
...
def draw_panel(surface: pygame.Surface, rect: pygame.Rect, alpha: int = 245) -> None:
    ...
    pygame.draw.rect(surface, (56, 75, 105), rect, 1, border_radius=12)
